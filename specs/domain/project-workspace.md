...
- Topbar: edit + **Archive** (Collection lifecycle via `POST /api/projects/:id/archive`) — not hard-delete; when the project has personas, **Ask all personas** as `@msqdx/ui` `Button` (subtle, sm) → `/chat?projectId=` (project ask-all; see `chat-workspace.md`)
