...
| `tavusLanguage` | Spoken CVI language: `de` or `en`. Null infers from magazine bio/location (`äöüß` / Deutschland / Germany / Österreich / Schweiz → `de`, else `en`). Magazine toggle Deutsch / English. |
...
6. `POST {TAVUS_API_BASE}/v2/conversations` with `face_id` and optional `pal_id` only (do **not** also send `replica_id` / `persona_id` — Tavus treats them as aliases and 400s). Include `properties.language` as the **full name** (`German` / `English` from `paths.tavusLanguageNames` — never `de`/`en`) plus `properties.participant_absent_timeout` / `participant_left_timeout` / `max_call_duration` from `paths` so idle rooms free the concurrency slot.
...
`conversational_context` on the conversation is **session-only** (stay in character / research call / this call’s language) — identity lives on the PAL.
