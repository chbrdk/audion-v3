...
15. **Destination quality (2026-08-26):** Hub/target `evaluate` scoring and `_href_key_match_score` apply `destination_quality_adjust`: heavily penalize newsletter / UTM / signup / promo CTAs; for service-intent tasks prefer help / Wartung / Ersatzteile / Beratung and compact `/…/service` hubs over marketing deep links under `/service/…`. URL-grounded batch buckets may mark newsletter/UTM landings as `goal_soft` (left home under service) vs `goal_ok` (help-quality surface).
