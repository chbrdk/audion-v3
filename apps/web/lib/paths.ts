...
import { buildChatProjectHref, buildChatTargetGroupHref } from './chat/tg-ask-all'
import {
...
export const paths = {
  ...
  tavusClientPath: 'apps/web/lib/tavus/client.ts',
  tavusVideoPanelPath: 'apps/web/components/tavus-video-panel.tsx',
  ...
  routes: {
    ...
    /** Project ask-all: /chat?projectId= (no personaId — share uses both). */
    chatProject: (projectId: string) => buildChatProjectHref(projectId),
    ...
    apiKnowledgeRagIngest: '/api/knowledge/rag/ingest',
    apiKnowledgeRagRetrieve: '/api/knowledge/rag/retrieve',
    apiKnowledgeRagDocuments: '/api/knowledge/rag/documents',
    apiKnowledgeRagDocument: (id: string) =>
      `/api/knowledge/rag/documents/${encodeURIComponent(id)}`,
  ...
  envTavusApiKey: 'TAVUS_API_KEY',
  envTavusApiBase: 'TAVUS_API_BASE',
  ...
  openRouterApiDefaultBase: 'https://openrouter.ai/api/v1',
  envKnowledgeRagEmbeddingModel: 'AUDION_RAG_EMBEDDING_MODEL',
  envKnowledgeRagEnabled: 'AUDION_RAG_ENABLED',
  knowledgeRagEmbeddingModel: 'openai/text-embedding-3-small',
  knowledgeRagEmbeddingDims: 1536,
  knowledgeRagChunkChars: 1800,
  knowledgeRagChunkOverlap: 200,
  knowledgeRagMaxChunksPerDoc: 200,
  knowledgeRagTopK: 5,
  knowledgeRagMinScore: 0.35,
  knowledgeRagEmbedBatchSize: 16,
