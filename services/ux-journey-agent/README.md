...
| `UX_JOURNEY_OPENAI_MODEL` | no | OpenAI model used as primary (when `OPENAI_API_KEY` is present and Anthropic is not) or as cross-provider fallback (default **`gpt-5.6-luna`** — Lab A/B 2026-08-03). Override to `gpt-5.4-mini` / `gpt-5.4-nano` if needed. |
