...
async def test_run_checks():
	...
	assert 'audion_agent_package' in checks
	assert checks['audion_agent_package']['status'] in ('ok', 'error')
...
def test_plan_actions_no_actions_needed():
	...
	checks = {
		'audion_agent_package': {'status': 'ok'},
		'browser': {'status': 'ok'},
	}
...
async def test_validate_setup():
	...
	assert 'audion_agent_import' in results
