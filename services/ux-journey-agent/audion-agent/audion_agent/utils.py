	...
	def __init__(
		...
		Args:
			...
			disabled: If True, signal handling is disabled and register() is a no-op.
					Useful when embedding audion-agent in applications that manage their own signals.
		"""
...
@cache
def get_audion_agent_version() -> str:
	"""Get the audion-agent package version using the same logic as Agent._set_audion_agent_version_and_source"""
	try:
		...
		version = str(get_version('audion-agent'))
		os.environ['LIBRARY_VERSION'] = version
		return version

	except Exception as e:
		logger.debug(f'Error detecting audion-agent version: {type(e).__name__}: {e}')
		return 'unknown'


async def check_latest_audion_agent_version() -> str | None:
	"""Check the latest version of audion-agent from PyPI asynchronously.
	...
	try:
		async with httpx.AsyncClient(timeout=3.0) as client:
			response = await client.get('https://pypi.org/pypi/audion-agent/json')
			if response.status_code == 200:
				data = response.json()
				latest_version = data['info']['version']
				if _is_newer_audion_agent_version(latest_version, get_audion_agent_version()):
...
def _is_newer_audion_agent_version(latest_version: str, current_version: str) -> bool:
	...
	try:
		...
		latest_key = _audion_agent_version_key(latest_version)
		current_key = _audion_agent_version_key(current_version)
		if latest_key is None or current_key is None:
			return latest_version != current_version
		return latest_key > current_key


def _audion_agent_version_key(version: str) -> tuple[tuple[int, ...], int, int, int] | None:
	"""Small PEP 440-ish fallback for audion-agent versions when packaging is unavailable."""
