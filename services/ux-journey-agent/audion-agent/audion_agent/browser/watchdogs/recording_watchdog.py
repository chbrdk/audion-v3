...
from audion_agent.browser.events import AgentFocusChangedEvent, BrowserConnectedEvent, BrowserStopEvent
from audion_agent.browser.profile import ViewportSize
from audion_agent.browser.video_recorder import VideoRecorderService
...
class RecordingWatchdog(BaseWatchdog):
	...
	async def start_recording(
		...
		if not recorder._is_active:
			raise RuntimeError(
				'Failed to initialize video recorder — ensure optional deps are installed (`pip install "audion-agent[video]"`).'
			)
