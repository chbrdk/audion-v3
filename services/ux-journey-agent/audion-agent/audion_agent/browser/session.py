...
from audion_agent.dom.views import DOMRect, EnhancedDOMTreeNode, TargetInfo
...
class BrowserSession(BaseModel):
	...
	def from_system_chrome(cls, profile_directory: str | None = None, **kwargs: Any) -> Self:
		...
		from audion_agent.skill_cli.utils import find_chrome_executable, get_chrome_profile_path, list_chrome_profiles
	...
	def list_chrome_profiles(cls) -> list[dict[str, str]]:
		...
		from audion_agent.skill_cli.utils import list_chrome_profiles
	...
	async def connect(self, cdp_url: str | None = None) -> Self:
		...
		if not self.cdp_url.startswith('ws'):
			...
			async with httpx.AsyncClient(timeout=httpx.Timeout(30.0), trust_env=not is_localhost) as client:
				...
				headers.setdefault('User-Agent', f'browser-use/{get_audion_agent_version()}')
