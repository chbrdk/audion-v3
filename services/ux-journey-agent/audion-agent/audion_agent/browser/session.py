...
from audion_agent.browser._cdp_timeout import TimeoutWrappedCDPClient
from audion_agent.browser.cloud.cloud import CloudBrowserAuthError, CloudBrowserClient, CloudBrowserError

# CDP logging is now handled by setup_logging() in logging_config.py
# It automatically sets CDP logs to the same level as audion_agent logs
from audion_agent.browser.cloud.views import CloudBrowserParams, CreateBrowserRequest, ProxyCountryCode
...
_UNSET: Any = object()
from audion_agent.browser.events import (
...
from audion_agent.browser.profile import BrowserProfile, ProxySettings
from audion_agent.browser.views import BrowserStateSummary, TabInfo
from audion_agent.dom.views import DOMRect, EnhancedDOMTreeNode, SerializedDOMState, TargetInfo
from audion_agent.observability import observe_debug
from audion_agent.utils import _log_pretty_url, create_task_with_error_handling, is_new_tab_page

if TYPE_CHECKING:
	from audion_agent.actor.page import Page
	from audion_agent.browser.demo_mode import DemoMode
	from audion_agent.browser.watchdogs.captcha_watchdog import CaptchaWaitResult
...
class BrowserSession(BaseModel):
	...
	def from_system_chrome(cls, profile_directory: str | None = None, **kwargs: Any) -> Self:
		...
		from audion_agent.browser.chrome import find_chrome_executable, get_chrome_profile_path, list_chrome_profiles
		...
		if profile_directory is None:
			if profiles:
				...
				logging.getLogger('audion_agent').info(
	...
	def list_chrome_profiles(cls) -> list[dict[str, str]]:
		...
		from audion_agent.browser.chrome import list_chrome_profiles
	...
	def logger(self) -> Any:
		...
		# 	self._logger = logging.getLogger(f'audion_agent.{self}')
		return logging.getLogger(f'audion_agent.{self}')
	...
	def model_post_init(self, __context) -> None:
		...
		from audion_agent.browser.watchdog_base import BaseWatchdog
	...
	async def kill(self) -> None:
		...
		from audion_agent.browser.events import SaveStorageStateEvent
		...
		try:
			...
			if self.is_local and not isinstance(e, (CloudBrowserAuthError, CloudBrowserError)):
				self.logger.warning(
					...
					'         Try: Browser(use_cloud=True)  |  Get an API key: https://cloud.audion-agent.com?utm_source=oss&utm_medium=browser_launch_failure'
				)
			raise
	...
	def _cloud_session_id_from_cdp_url(self) -> str | None:
		...
		match = re.match(r'^([0-9a-fA-F-]{36})\.cdp\d+\.audion-agent\.com$', host)
	...
	async def new_page(self, url: str | None = None) -> 'Page':
		...
		from audion_agent.actor.page import Page as Target
	...
	async def get_pages(self) -> list['Page']:
		...
		# Import here to avoid circular import
		from audion_agent.actor.page import Page as PageActor
	...
	async def attach_all_watchdogs(self) -> None:
		...
		from audion_agent.browser.watchdogs.aboutblank_watchdog import AboutBlankWatchdog
		from audion_agent.browser.watchdogs.captcha_watchdog import CaptchaWatchdog

		# from audion_agent.browser.crash_watchdog import CrashWatchdog
		from audion_agent.browser.watchdogs.default_action_watchdog import DefaultActionWatchdog
		from audion_agent.browser.watchdogs.dom_watchdog import DOMWatchdog
		from audion_agent.browser.watchdogs.downloads_watchdog import DownloadsWatchdog
		from audion_agent.browser.watchdogs.har_recording_watchdog import HarRecordingWatchdog
		from audion_agent.browser.watchdogs.local_browser_watchdog import LocalBrowserWatchdog
		from audion_agent.browser.watchdogs.permissions_watchdog import PermissionsWatchdog
		from audion_agent.browser.watchdogs.popups_watchdog import PopupsWatchdog
		from audion_agent.browser.watchdogs.recording_watchdog import RecordingWatchdog
		from audion_agent.browser.watchdogs.screenshot_watchdog import ScreenshotWatchdog
		from audion_agent.browser.watchdogs.security_watchdog import SecurityWatchdog
		from audion_agent.browser.watchdogs.storage_state_watchdog import StorageStateWatchdog
	...
	async def connect(self, cdp_url: str | None = None) -> Self:
		...
		if not self.cdp_url.startswith('ws'):
			...
			async with httpx.AsyncClient(timeout=httpx.Timeout(30.0), trust_env=not is_localhost) as client:
				...
				from audion_agent.utils import get_audion_agent_version

				headers.setdefault('User-Agent', f'audion-agent/{get_audion_agent_version()}')
		...
		try:
			...
			from audion_agent.browser.session_manager import SessionManager
			...
			from audion_agent.utils import is_new_tab_page
	...
	async def navigate_to(self, url: str, new_tab: bool = False) -> None:
		...
		"""
		from audion_agent.browser.events import NavigateToUrlEvent
	...
	async def get_dom_element_at_coordinates(self, x: int, y: int) -> EnhancedDOMTreeNode | None:
		...
		"""
		from audion_agent.dom.views import NodeType
	...
	def find_file_input_near_element(
		...
		"""
		from audion_agent.dom.views import EnhancedDOMTreeNode
	...
	async def remove_highlights(self) -> None:
		...
		try:
			async with asyncio.timeout(3.0):
				...
				script = """
				(function() {
					// Remove all audion-agent highlight elements
					const highlights = document.querySelectorAll('[data-audion-agent-highlight]');
					console.log('Removing', highlights.length, 'audion-agent highlight elements');
					highlights.forEach(el => el.remove());

					// Also remove by ID in case selector missed anything
					const highlightContainer = document.getElementById('audion-agent-debug-highlights');
					...
					const orphanedTooltips = document.querySelectorAll('[data-audion-agent-highlight="tooltip"]');
	...
	async def highlight_interaction_element(self, node: 'EnhancedDOMTreeNode') -> None:
		...
		try:
			...
			(function() {{
				...
				container.setAttribute('data-audion-agent-interaction-highlight', 'true');
	...
	async def highlight_coordinate_click(self, x: int, y: int) -> None:
		...
		try:
			...
			(function() {{
				...
				// Create container
				const container = document.createElement('div');
				container.setAttribute('data-audion-agent-coordinate-highlight', 'true');
				container.style.cssText = `
					position: absolute;
					left: ${{x + scrollX}}px;
					top: ${{y + scrollY}}px;
	...
	async def add_highlights(self, selector_map: dict[int, 'EnhancedDOMTreeNode']) -> None:
		...
		try:
			...
			(function() {{
				...
				const existingContainer = document.getElementById('audion-agent-debug-highlights');
				...
				const strayHighlights = document.querySelectorAll('[data-audion-agent-highlight]');
				...
				const container = document.createElement('div');
				container.id = 'audion-agent-debug-highlights';
				container.setAttribute('data-audion-agent-highlight', 'container');
				...
				interactiveElements.forEach((element, index) => {{
					...
					highlight.setAttribute('data-audion-agent-highlight', 'element');
