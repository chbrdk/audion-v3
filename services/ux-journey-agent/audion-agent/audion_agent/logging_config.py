...
def setup_logging(stream=None, log_level=None, force_setup=False, debug_log_file=None, info_log_file=None):
	...
	if logging.getLogger().hasHandlers() and not force_setup:
		return logging.getLogger('audion_agent')
	...
	class BrowserUseFormatter(logging.Formatter):
		...
		def format(self, record):
			...
			if self.log_level > logging.DEBUG and isinstance(record.name, str) and record.name.startswith('audion_agent.'):
				...
				if 'Agent' in record.name:
					...
					record.name = 'dom'
				elif record.name.startswith('audion_agent.'):
					# For other audion_agent modules, use the last part
	...
	# Configure audion_agent logger
	audion_agent_logger = logging.getLogger('audion_agent')
	audion_agent_logger.propagate = False  # Don't propagate to root logger
	audion_agent_logger.addHandler(console)
	for handler in file_handlers:
		audion_agent_logger.addHandler(handler)
	audion_agent_logger.setLevel(effective_log_level)
...
def setup_log_pipes(session_id: str, base_dir: str | None = None):
	...
	for name in ['audion_agent.agent', 'audion_agent.tools']:
	...
	for name in ['bubus', 'audion_agent.browser.session']:
		logger = logging.getLogger(name)
