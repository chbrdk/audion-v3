...
def _load_skill_text_from_package() -> str:
	from audion_agent.skills.audion_agent import skill_text
...
def _install_audion_agent_tool() -> None:
	uv = shutil.which('uv')
	if not uv:
		raise RuntimeError('Installing the Browser Use skill requires `uv`. Install uv, then rerun `audion-agent skill install`.')

	result = subprocess.run([uv, 'tool', 'install', '--python', '3.12', '--upgrade', '--force', 'audion-agent'])
	if result.returncode != 0:
		raise RuntimeError('Failed to install audion-agent with `uv tool install --python 3.12 --upgrade --force audion-agent`.')
...
def _load_skill_text_from_browser_harness_cli() -> str:
	...
	from audion_agent.skills.audion_agent import as_audion_agent_skill

	return as_audion_agent_skill(result.stdout)


def _build_parser() -> argparse.ArgumentParser:
	parser = argparse.ArgumentParser(
		prog='audion-agent skill',
	...
	install.add_argument(
		'--no-install',
		action='store_true',
		help='Skip uv tool install/upgrade and use the existing audion-agent command or package',
	)
