...
class CLIMCPServer:
	...
	def __init__(self):
		self.server: Server = Server('audion-agent')
			...
			try:
				await self.server.run(
					...
					InitializationOptions(
						...
						server_version=get_audion_agent_version(),
...
async def main():
	...
	os.environ.setdefault('BH_CLIENT', 'audion-agent-mcp')
	server = CLIMCPServer()
	await server.run()
