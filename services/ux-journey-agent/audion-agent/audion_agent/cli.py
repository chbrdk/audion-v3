...
if 'init' in sys.argv:
	from audion_agent.init_cmd import INIT_TEMPLATES
...
if '--template' in sys.argv:
	...
	output_path = Path(output) if output else Path.cwd() / f'audion_agent_{template}.py'
...
from audion_agent import Agent, Controller
from audion_agent.agent.views import AgentSettings
from audion_agent.browser import BrowserProfile, BrowserSession
from audion_agent.logging_config import addLoggingLevel
from audion_agent.telemetry import CLITelemetryEvent, ProductTelemetry
...
class BrowserUseApp(App):
	...
	def setup_richlog_logging(self) -> None:
		...
		class BrowserUseFormatter(logging.Formatter):
			def format(self, record):
				# if isinstance(record.name, str) and record.name.startswith('audion_agent.'):
		...
		# Configure audion_agent logger and all its sub-loggers
		audion_agent_logger = logging.getLogger('audion_agent')
		audion_agent_logger.propagate = False  # Don't propagate to root logger
		audion_agent_logger.handlers = [log_handler]  # Replace any existing handlers
		audion_agent_logger.setLevel(root.level)

		# Also ensure agent loggers go to the main output
		# Use a wildcard pattern to catch all agent-related loggers
		for logger_name in ['audion_agent.Agent', 'audion_agent.controller', 'audion_agent.agent', 'audion_agent.agent.service']:
		...
		for name, logger in logging.Logger.manager.loggerDict.items():
			if isinstance(name, str) and 'audion_agent.Agent' in name:
	...
	def on_mount(self) -> None:
		...
		logger = logging.getLogger('audion_agent.on_mount')
	...
	def run_task(self, task: str) -> None:
		...
		# Get the logger
		logger = logging.getLogger('audion_agent.app')
...
async def textual_interface(config: dict[str, Any]):
	...
	# Prevent audion_agent from setting up logging at import time
	os.environ['BROWSER_USE_SETUP_LOGGING'] = 'false'

	logger = logging.getLogger('audion_agent.startup')
	...
	try:
		...
		try:
			from audion_agent.logging_config import setup_log_pipes
...
async def run_auth_command():
	...
	from audion_agent.sync.auth import DeviceAuthClient
	...
	try:
		...
		from audion_agent.sync.service import CloudSync
		...
		try:
			...
			from audion_agent.utils import create_task_with_error_handling
		...
		if task_id and sync_service:
			try:
				from audion_agent.agent.cloud_events import UpdateAgentTaskEvent
...
@click.option('--output', '-o', type=click.Path(), help='Output file path for template (default: audion_agent_<template>.py)')
...
def run_main_interface(ctx: click.Context, debug: bool = False, **kwargs):
	...
	if kwargs.get('mcp'):
		...
		try:
			...
			pass
		# Run as MCP server
		from audion_agent.mcp.server import main as mcp_main
...
def _run_template_generation(template: str, output: str | None, force: bool):
	...
	if output:
		...
		output_path = Path.cwd() / f'audion_agent_{template}.py'
...
@click.option(
	...
	'-o',
	type=click.Path(),
	help='Output file path (default: audion_agent_<template>.py)',
)
@click.option(
	'--force',
	'-f',
