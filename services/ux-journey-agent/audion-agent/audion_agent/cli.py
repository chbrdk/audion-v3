...
def _audion_agent_version() -> str:
	try:
		return version('audion-agent')
...
def _set_harness_client_env() -> None:
	...
	os.environ['BH_CLIENT'] = 'audion-agent-cli'
	os.environ['BH_CLIENT_VERSION'] = _audion_agent_version()
...
def _run_mcp_server() -> None:
	_run_mcp_stdio_server('audion_agent.mcp.server')


def _run_cli_mcp_server() -> None:
	_run_mcp_stdio_server('audion_agent.mcp.cli_mcp')


def _run_install_command(argv: list[str]) -> int:
	if any(arg in {'-h', '--help'} for arg in argv):
		print('usage: audion-agent install')
		print()
	...
	if result.returncode == 0:
		...
		print('Ready to use. Run: uvx audion-agent')
		return 0
...
def _run_init_command(argv: list[str]) -> int | None:
	from audion_agent.init_cmd import main as init_main
...
def _as_audion_agent_cli_text(text: str) -> str:
	return text.replace('Browser Harness', 'Browser Use').replace('browser-harness', 'audion-agent')
...
def _normalize_captured_cli_output(func, argv: list[str]) -> int:
	...
	err = stderr.getvalue()
	if out:
		print(_as_audion_agent_cli_text(out), end='')
	if err:
		print(_as_audion_agent_cli_text(err), end='', file=sys.stderr)
	if result is None:
		return 0
	if isinstance(result, int):
		return result
	if isinstance(result, str):
		print(_as_audion_agent_cli_text(result), file=sys.stderr)
		return 1
	return 1
...
def _patch_browser_harness_cli_text() -> None:
	...
	run.HELP = _as_audion_agent_cli_text(run.HELP)
	run.USAGE = _as_audion_agent_cli_text(run.USAGE)
...
def _run_browser_harness() -> int | None:
	...
	if args and args[0] == 'doctor' and args[1:]:
		if args[1:] in (['--help'], ['-h']):
			print('usage: audion-agent doctor [--fix-snap]')
			return 0
		if args[1:] != ['--fix-snap']:
			print('usage: audion-agent doctor [--fix-snap]', file=sys.stderr)
			sys.exit(2)
...
_LEGACY_HINTS: dict[str, str] = {
	...
	'close': '# restart the local daemon with `audion-agent --reload`; stop cloud browsers with stop_remote_daemon(name)',
	...
	'cloud': '# authenticate with `audion-agent auth login`, then start_remote_daemon("<name>")',
	'daemon': '# the daemon starts automatically on every call; restart it with `audion-agent --reload`',
	'record': '# session recording was removed; use capture_screenshot() per step',
	'mcp': '# MCP server mode is now the --mcp flag: `audion-agent --mcp`',
	...
	'-c': "# pipe code on stdin instead: echo 'print(page_info())' | audion-agent",
	'--code': "# pipe code on stdin instead: echo 'print(page_info())' | audion-agent",
}
...
Read the full interface now:   audion-agent skill show
Install the CLI 3.0 skill (upgrades the CLI and replaces any pre-existing audion-agent skill):
                               audion-agent skill install
Then load the freshly installed audion-agent skill, and follow it to use the latest CLI.
Skill reference:               https://github.com/audion-agent/audion-agent/blob/main/audion_agent/skills/audion-agent/SKILL.md
Health check:                  audion-agent --doctor"""
...
def _legacy_migration_message(command: str) -> str:
	...
	if hint.startswith('#'):
		...
		replacement = f"  audion-agent <<'PY'\n  {hint}\n  PY"
	return f"""The audion-agent CLI changed in 3.0, and '{command}' was removed.
...
def _unknown_helper_message(name: str) -> str:
	return f"""'{name}' is not defined in the audion-agent CLI.
...
You can also paste this into your agent to get started:
  ...
  Install or upgrade audion-agent to the latest stable version with uv using
  Python 3.12, register the skill from `audion-agent skill`, and connect it to
  my browser. Follow https://github.com/audion-agent/audion-agent if setup or
  connection fails.

More:
  audion-agent --doctor     check install, daemon, and browser health
  audion-agent --help       full command list
  docs: https://github.com/audion-agent/audion-agent/blob/main/audion_agent/skills/audion-agent/SKILL.md"""

_EMPTY_STDIN_MESSAGE = """audion-agent received empty stdin. This CLI executes Python piped on stdin:
  audion-agent <<'PY'
  print(page_info())
  PY"""
...
def _dispatch(args: list[str]) -> tuple[int | None, str]:
	...
	if args and args[0] == 'skill':
		from audion_agent.skills.install import handle as handle_skill_command
...
def audion_agent_tui_main() -> int | None:
	print('audion-agent-tui is deprecated; use audion-agent instead.', file=sys.stderr)
	return main()
