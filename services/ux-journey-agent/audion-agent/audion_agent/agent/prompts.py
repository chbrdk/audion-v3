...
from audion_agent.browser.views import PLACEHOLDER_4PX_SCREENSHOT
from audion_agent.dom.views import NodeType, SimplifiedNode
from audion_agent.llm.messages import ContentPartImageParam, ContentPartTextParam, ImageURL, SystemMessage, UserMessage
from audion_agent.observability import observe_debug
from audion_agent.utils import is_new_tab_page, sanitize_surrogates

if TYPE_CHECKING:
	from audion_agent.agent.views import AgentStepInfo
...
class SystemPrompt:
	def __init__(
		...
		is_audion_agent_model: bool = False,
		...
		self.is_audion_agent_model = is_audion_agent_model
	...
	def _load_prompt_template(self) -> None:
		...
		try:
			...
			if self.is_audion_agent_model:
				if self.flash_mode:
					template_filename = 'system_prompt_audion_agent_flash.md'
				elif self.use_thinking:
					template_filename = 'system_prompt_audion_agent.md'
				else:
					template_filename = 'system_prompt_audion_agent_no_thinking.md'
			...
			with (
				importlib.resources.files('audion_agent.agent.system_prompts')
