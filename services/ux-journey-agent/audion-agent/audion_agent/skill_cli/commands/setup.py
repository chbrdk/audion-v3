...
async def run_checks() -> dict[str, Any]:
	...
	try:
		...
		checks['audion_agent_package'] = {
			'status': 'ok',
			'message': f'browser-use {audion_agent.__version__}'
			if hasattr(audion_agent, '__version__')
...
async def validate_setup() -> dict[str, Any]:
	...
	# Check imports
	try:
		import audion_agent  # noqa: F401

		results['audion_agent_import'] = 'ok'
	except ImportError:
		results['audion_agent_import'] = 'failed'
