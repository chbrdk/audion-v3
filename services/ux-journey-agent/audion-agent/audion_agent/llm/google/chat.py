...
from audion_agent.llm.exceptions import ModelOutputTruncatedError, ModelProviderError
from audion_agent.llm.google.serializer import GoogleMessageSerializer
...
class ChatGoogle(BaseChatModel):
	...
	def logger(self) -> logging.Logger:
		...
		return logging.getLogger(f'audion_agent.llm.google.{self.model}')

	def _get_http_options(self) -> dict[str, Any]:
		"""Get http options with the default headers set."""
		try:
			bu_version = importlib.metadata.version('audion-agent')
		except importlib.metadata.PackageNotFoundError:
			bu_version = 'unknown'

		header_value = f'audion-agent/{bu_version}'
