...
from audion_agent.telemetry.views import BaseTelemetryEvent
from audion_agent.utils import singleton
...
def _machine_fingerprint() -> str | None:
	...
	if (node >> 40) & 0x01:  # multicast bit set: getnode() failed and returned a random id
		return None
	return 'bu_' + hashlib.sha256(f'audion-agent:{node}:{socket.gethostname()}'.encode()).hexdigest()[:32]
...
class ProductTelemetry:
	...
	def __init__(self) -> None:
		...
		if telemetry_disabled:
			...
			logger.info('Using anonymized telemetry, see https://docs.audion-agent.com/development/monitoring/telemetry.')
