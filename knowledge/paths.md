...
- Tavus env: `TAVUS_API_KEY` (`paths.envTavusApiKey`) · `TAVUS_API_BASE` (`paths.envTavusApiBase`, default `paths.tavusApiDefaultBase`) · conversations `paths.tavusConversationsPath` (`/v2/conversations`) · end suffix `paths.tavusConversationEndSuffix` (`/end`) · name prefix `paths.tavusConversationNamePrefix`
