...
- Collection team (Plexon SSOT): `knowledge/collection-team-plexon.md` · BFF `/api/projects/:id/members` · `/invites` · `/sync-collection-members` · outbound mail = Plexon only (`plexon-v3/specs/domain/transactional-email.md`)
...
- Shell back: `ShellBackButton` via `AppFrame.backCorner` (`router.back()`, `nav.back`) — DS `msqdx-ui`
...
- Persona chat human-likeness eval (bilingual DE/EN, enterprise gate): `scripts/eval-persona-chat.mjs` · lib `apps/web/lib/chat/eval/` · catalog `apps/web/lib/chat/eval/catalog.json` · env `AUDION_API_TOKEN` + optional `AUDION_V3_BASE_URL` / `EVAL_PERSONA_ID` / `EVAL_PROJECT_ID` / `EVAL_CASE_ID` / `EVAL_PASS_RATE_GATE` · out `.tmp/persona-chat-eval/latest.json` · spec `specs/domain/persona-chat-eval.md` · `knowledge/persona-chat-eval.md` · spot-check `knowledge/persona-chat-spot-check-2026-09-20.md` · Employer spot-check `knowledge/persona-chat-employer-spot-check-2026-09-20.md` · **automatic deploy gate** `knowledge/persona-chat-quality-gate.md` · `npm run test:persona-chat` (Dockerfile)
- Humanize persona replies without dropping traits: `knowledge/persona-chat-humanize-2026-09-20.md` · soft filter `apps/web/lib/chat/humanize-reply.ts` · greeting/few-shot/elicitation in `adaptive-persona-chat-prompt.ts` · locale few-shots via `detectChatLocale`
- Coolify REST env ops (MCP hides values; REST `/applications/{uuid}/envs` returns them): `knowledge/coolify-rest-env-ops-2026-09-20.md`
  ...
  - File upload on dossiers (`.docx` / `.pdf` / `.pptx` / `.md`): `apiProjectKnowledgeUpload` · `apiPersonaKnowledgeUpload` · `apiTargetGroupKnowledgeUpload` → chapter/entry + auto RAG · extract `lib/knowledge/docx-to-knowledge.ts`
...
- Native AI runtime: `NEXT_AI_RUNTIME` (`stub` | `native` | `auto`) · `OPENAI_API_KEY` · `AI_OPENAI_MODEL` (default **`gpt-6-luna`**, SoT `paths.aiOpenAiModel`) — `knowledge/ai-native-2026.md` · bump note `knowledge/openai-model-gpt-6-luna-2026-09-23.md`
...
- UX Journey OpenAI: `UX_JOURNEY_OPENAI_MODEL` (default **`gpt-6-luna`**, SoT `paths.uxJourneyOpenAiModel`) — agent `main.py` / Dockerfile / local lab serve · A/B: `knowledge/lab-ab-nano-mini-luna-2026-08-03.md`
...
- UX Journey destination quality: `knowledge/ux-agent-destination-quality-2026-08-26.md` · service help ≫ newsletter/UTM CTAs · bucket `goal_soft` · quality-smoke `knowledge/ueq-ebike-runs/2026-08-26-quality/` · buckets `knowledge/ux-journey-fail-buckets-quality-smoke-2026-08-26.json`
- UEQ / post-hoc infer: `UEQ_INFER_MODEL` → else `UX_JOURNEY_OPENAI_MODEL` → **`gpt-6-luna`** (never `gpt-4o-mini`) · `scripts/infer-ueq-ebike-scores.py`
...
- Jev System One (shadow): `specs/domain/jev-decisions.md` · `apps/web/lib/jev/` · OpenRouter Decisions `POST {OPENROUTER_API_BASE_URL}/api/alpha/decisions` · model `JEV_MODEL_ID` default `typesafe/jev-1.13` · env `OPENROUTER_API_KEY` · `OPENROUTER_API_BASE_URL` · `JEV_SHADOW_ENABLED` · `JEV_SHADOW_<USE_CASE>` · `JEV_ACT_<USE_CASE>` (default off — no Act this wave) · `JEV_TIMEOUT_MS` · wave notes `knowledge/jev-audion-shadow-wave-2026-09-26.md` · suite catalog plexon-v3 `specs/domain/jev-use-case-catalog.md`
