...
- UX Test Flows (product templates): `paths.routes.studiesFlows` (`/studies/flows`) · detail canvas+list+**Protokoll** · in-flow **Testen** (create+start+poll) · Save/load · Live-Gate `gateSignals` + mid-run replan · `apiStudiesFromFlow` · `apiStudiesFlowsSaved` · `apiStudyWaveStart` · `apiUxJourneyAgentRun` · lib `paths.uxTestFlowsLibPath` · store `paths.uxFlowStorePath` · db `paths.uxFlowDbPath` · replan `paths.uxFlowReplanPath` · protocol `paths.uxFlowModeratedProtocolPath` · progress `paths.uxFlowRunProgressPath` · canvas map `apps/web/lib/ux-flow-canvas.ts` · model `paths.uxTestFlowModelSpecPath` · scenarios `paths.uxTestFlowScenariosSpecPath`
- UX Study fixtures/store: `apps/web/lib/fixtures/ux-studies.ts` · `ux-study-store.ts`
- UX saved flows: Postgres `ux_saved_flows` via `apps/web/lib/db/ux-saved-flows.ts` when `DATABASE_URL` set; else in-memory `ux-flow-store.ts` · API `GET/POST /api/studies/flows/saved` · `GET/DELETE …/saved/[id]`
- Moderated protocol (no agent): `?view=protocol` · `paths.routes.studiesFlowProtocol`
