...
- Dockerfile runs: `DATABASE_URL= npm run test:persona-chat` before `npm run build` (`.dockerignore` must not exclude `__tests__` / vitest config; clear DB URL so fixture stores stay in-memory under Coolify)
